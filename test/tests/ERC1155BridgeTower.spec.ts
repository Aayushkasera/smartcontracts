...
describe("ERC1155BridgeTower", () => {
  ...
  describe("WhitelistableUpgradeable", () => {
    describe("setSecuritizeRegistryProxy", () => {
      ...
      // If contractsRegistry has remove function, call it too
      // await contractsRegistry.connect(alice).removeContract(alice.address);  
  ...
  describe("ERC1155BridgeTower", () => {
    ...
    describe("safeTransferFrom", () => {
      ...
      it("should fail if user is trying to transfer more than unlocked tokens amount", async () => {
        await securitizeRegistry.connect(alice).addWallet(exchangeV2Proxy.address);
        await securitizeRegistry.connect(alice).addWallet(transferProxy.address);
        await securitizeRegistry.connect(alice).addWallet(erc20TransferProxy.address);

        // ✅ ADD EXCHANGE AS OPERATOR TO BOTH PROXIES
        await transferProxy.connect(alice).addOperator(exchangeV2Proxy.address);
        await erc20TransferProxy.connect(alice).addOperator(exchangeV2Proxy.address);
    ...
    // Get signatures properly
    const leftOrderSignature = await sign(leftOrder, alice, exchangeV2Proxy.address);
    const rightOrderSignature = await sign(rightOrder, bob, exchangeV2Proxy.address);

    // Debug signatures
    console.log("Left signature:", leftOrderSignature);
    console.log("Right signature:", rightOrderSignature);
...
console.log('res',res);
      ...
      it("should lock proper amount of tokens", async () => {
        ...
        const prevBobLockedBalance: BigNumber = await erc1155BridgeTowerProxy
          .connect(bob)
          .getLockedAmount(bob.address, tokenID1);
          console.log('prevBobLockedBalance',prevBobLockedBalance);
    ...
    describe("getLocksInfo", () => {
      it("should return proper locks info", async () => {
        const locksInfo: any = await erc1155BridgeTowerProxy
          ...
          console.log(locksInfo[0],'locksInfo');
