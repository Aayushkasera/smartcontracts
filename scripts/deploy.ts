...
async function main() {
  ...
  for (let i: number = 0; i < partners.length; ++i) {
    tx = await securitizeRegistry.addWallet(partners[i]);
  ...
  for (let i: number = 0; i < initialWhitelist.length; ++i) {
    tx = await securitizeRegistry.addWallet(initialWhitelist[i]);
   ...
   * ERC1155BridgeTower
   */
  const ERC1155BridgeTower: ContractFactory =
    await ethers.getContractFactory("ERC1155BridgeTower");
